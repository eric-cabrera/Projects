namespace Assurity.Kafka.Managers
{
    ...
    public class SysZ9ProcessEventManager : ISysZ9ProcessEventManager
    {
        public SysZ9ProcessEventManager(
            IGlobalDataAccessor globalDataAccessor,
            IConsumerHierarchyEngine hierarchyEngine,
            ILogger<SysZ9ProcessEventManager> logger,
        ...
        private ILogger<SysZ9ProcessEventManager> Logger { get; }
        ...
        [Transaction]
        public async Task ProcessEvent(SysZ9Process sysZ9Process)
        {
            var policyNumber = await GlobalDataAccessor
                .GetPolicyNumber(sysZ9Process.NBFOLDEROBJID);

            if (string.IsNullOrWhiteSpace(policyNumber))
            {
                Logger.LogWarning("No policy numbers were found in Global associated with Folder Id: {folderId}", sysZ9Process.NBFOLDEROBJID);
            ...
            if (removedAgents != null)
            {
                foreach (var agent in removedAgents)
                {
                    if (removedAgents.Contains(agent))
                    {
                        ...
                        {
                            Logger.LogError("Processing event for SysZ9Process PolicyNumber: Failed to remove {PolicyNumber} from AgentPolicyAccess for {Agent}", policyNumber, agent);
                        }
                    }
                }
            }
            ...
            if (addedAgents != null)
            {
                foreach (var agent in addedAgents)
                {
                    if (addedAgents.Contains(agent))
                    {
                        ...
                        if (agentPolicyAccessId == 0)
                        {
                            Logger.LogError("Processing event for SysZ9Process PolicyNumber: Failed to add {PolicyNumber} to AgentPolicyAccess for {Agent}", policyNumber, agent);
                        }
                    }
                }
            }
