namespace Assurity.Kafka.Managers
{
    ...
    public class SysACAgentDataEventManager : ISysACAgentDataEventManager
    {
        public SysACAgentDataEventManager(
            IGlobalDataAccessor globalDataAccessor,
            IConsumerHierarchyEngine hierarchyEngine,
            ILogger<SysACAgentDataEventManager> logger,
        ...
        private ILogger<SysACAgentDataEventManager> Logger { get; }
        ...
        [Transaction]
        public async Task ProcessEvent(SysACAgentData sysACAgentData)
        {
            var policyNumber = await GlobalDataAccessor
                .GetPolicyNumber(sysACAgentData.FOLDERID);

            if (string.IsNullOrWhiteSpace(policyNumber))
            {
                Logger.LogWarning("No policy numbers were found in Global associated with Folder Id: {folderId}", sysACAgentData.FOLDERID);
            ...
            if (removedAgents != null)
            {
                foreach (var agent in removedAgents)
                {
                    if (removedAgents.Contains(agent))
                    {
                        ...
                        {
                            Logger.LogError("Processing event for SysAgentData PolicyNumber: Failed to remove {PolicyNumber} from AgentPolicyAccess for {Agent}", policyNumber, agent);
                        }
                    }
                }
            }
            ...
            if (addedAgents != null)
            {
                foreach (var agent in addedAgents)
                {
                    if (addedAgents.Contains(agent))
                    {
                        ...
                        if (agentPolicyAccessId == 0)
                        {
                            Logger.LogError("Processing event for SysAgentData PolicyNumber: Failed to add {PolicyNumber} to AgentPolicyAccess for {Agent}", policyNumber, agent);
                        }
                    }
                }
            }
