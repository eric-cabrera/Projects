...
", conn))
	{
		using (SqlDataReader reader = cmd.ExecuteReader())
		{
			while (reader.Read())
			{
				...
				owner.Participant.Address.Country = parseCountry((string)reader["ParticipantAdressCountry"]);
...
", conn))
	{
		using (SqlDataReader reader = cmd.ExecuteReader())
		{
			while (reader.Read())
			{
				...
				payor.Participant.Address.Country = parseCountry((string)reader["ParticipantAdressCountry"]);
...
", conn))
	{
		using (SqlDataReader reader = cmd.ExecuteReader())
		{
			while (reader.Read())
			{
				...
				insured.Participant.Address.Country = parseCountry((string)reader["ParticipantAdressCountry"]);
